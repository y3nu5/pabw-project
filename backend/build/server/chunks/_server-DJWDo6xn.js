...
function isOllamaUnavailable(error) {
  const message = error && typeof error === "object" && "message" in error ? toText(error.message).toLowerCase() : "";
  return message.startsWith("ollama_unavailable:") || message.includes("connect") || message.includes("econnrefused") || message.includes("enotfound") || message.includes("ehostunreach");
}
async function callOllama(messages) {
  ...
  console.log("[DEBUG callOllama] API Key set:", !!private_env.OLLAMA_API_KEY);
  const headers = {
    "Content-Type": "application/json"
  };
  if (private_env.OLLAMA_API_KEY) {
    headers["Authorization"] = `Bearer ${private_env.OLLAMA_API_KEY.trim()}`;
  }
  try {
    ...
    const message = error && typeof error === "object" && "message" in error ? toText(error.message) : "fetch error";
...
export { POST };
//# sourceMappingURL=_server-DJWDo6xn.js.map
